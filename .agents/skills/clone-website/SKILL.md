---
name: clone-website
description: Reverse-engineer and clone one or more websites in one shot — extracts assets, CSS, and content section-by-section into a production-ready Next.js application. Use this whenever the user wants to clone, replicate, rebuild, reverse-engineer, or copy any website. Also triggers on phrases like "make a copy of this site", "rebuild this page", "pixel-perfect clone". Provide one or more target URLs as arguments.
...
When multiple URLs are provided, process them sequentially or in parallel where resources allow, while keeping each site's extraction artifacts isolated in dedicated folders (for example, `docs/research/<hostname>/`).

This is not a two-phase process (inspect then build). You are a **foreman walking the job site** — as you inspect each section of the page, you write a detailed specification to a file. Extraction is meticulous and produces auditable artifacts that guide component implementation.
...
3. Verify that a Next.js project exists and can build: `npm run build`. The Next.js + shadcn/ui + Tailwind v4 scaffold should already be in place. If not, tell the user to set it up first or create it as needed.
4. Create the output directories if they don't exist: `docs/research/`, `docs/research/components/`, `docs/design-references/`. For multiple clones, also prepare per-site folders like `docs/research/<hostname>/` and `docs/design-references/<hostname>/`.
5. When working with multiple sites in one command, determine whether to process them sequentially or in parallel based on available resources.
...
**Fonts** — Inspect `<link>` tags for Google Fonts or self-hosted fonts. Check computed `font-family` on key elements (headings, body, code, labels). Document every family, weight, and style actually used. Configure them in `app/layout.tsx` using `next/font/google` or `next/font/local`.

**Colors** — Extract the site's color palette from computed styles across the page. Update `app/globals.css` with the target's actual colors in the `:root` and `.dark` CSS variable blocks. Map them to shadcn's token names (background, foreground, primary, muted, etc.) where they fit. Add custom properties for colors that don't map to shadcn tokens.
...
3. **Create TypeScript interfaces** in `types/` for the content structures you've observed
4. **Extract SVG icons** — find all inline `<svg>` elements on the page, deduplicate them, and save as named React components in `components/icons.tsx`. Name them by visual function (e.g., `SearchIcon`, `ArrowRightIcon`, `LogoIcon`).
5. **Download global assets** — download all images, videos, and other binary assets from the page to `public/`. Preserve meaningful directory structure. You can use browser MCP to extract asset URLs and fetch them programmatically.
...
// Returns a JSON structure with all images, videos, backgrounds, and fonts on the page
JSON.stringify({
...
Use browser MCP or Node.js tools to download assets to `public/` with proper error handling and batching.
...
## Overview
- **Target file:** `components/<ComponentName>.tsx`
...
Based on complexity, implement components in `components/`:

**Simple section** (1-2 sub-components): Create one component file for the entire section.

**Complex section** (3+ distinct sub-components): Break it up. Create one component file per sub-component, plus one wrapper component that imports and composes them.

**Implementation checklist for each component:**
- Use the full spec file contents to guide implementation
- Reference the section screenshot in `docs/design-references/`
- Import shared components (`icons.tsx`, `cn()`, shadcn primitives)
- Target file path: `components/<SectionName>.tsx`
- Verify TypeScript compilation: `npx tsc --noEmit`
- Implement responsive behavior at the specified breakpoints
- After each component is completed, verify the build still passes: `npm run build`

The extract → spec → implement cycle continues until all sections are built.

## Phase 4: Page Assembly

After all sections are built and merged, wire everything together in `app/page.tsx`:
...
Before implementing ANY component from a spec file, verify you can check every box. If you can't, go back and extract more.
...
- [ ] The component implementation will be straightforward; if the spec is overly complex, break the section into smaller pieces
...
- **Don't build components without a solid spec file.** The spec file forces exhaustive extraction and creates an auditable artifact. Without it, you'll guess to fill gaps and produce incomplete work.
